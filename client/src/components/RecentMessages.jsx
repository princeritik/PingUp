...
export default function RecentMessages() {
    ...
    return (
        <div className='bg-white max-w-xs mt-4 p-4 min-h-20 rounded-md text-xs shadow text-slate-800'>
            <h3 className='font-semibold text-slate-800  mb-4 ' >Recent Messages</h3>
            <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar' >
                {
                    messages.map((message, index) => (
                        <Link key={index} to={`/messages/${message.from_user_id._id}`} className='flex flex-start gap-2 py-2 hover:bg-slate-100' >
